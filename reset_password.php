    ...
    if ($result->num_rows === 1) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (strlen($password) < 8) {
                $message = "<p class='text-red-600 text-center font-semibold'>Password must be at least 8 characters long.</p>";
            } else {
                ...
                $message = "
                    <p class='text-green-600 text-center font-semibold'>
        ...
        $message = "<p class='text-red-600 text-center'>Invalid or expired reset link.</p>";
    }
} else {
    $message = "<p class='text-red-600 text-center'>Missing reset token.</p>";
}
?>
<!DOCTYPE html>
<html lang="en">
...
<main class="flex-grow flex items-center justify-center px-4 py-12">
  <form method="POST" class="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-lg w-full max-w-md">
    <h2 class="text-2xl font-semibold mb-4 text-center">Reset Password</h2>
    <p class="text-sm text-gray-600 dark:text-gray-400 mb-6 text-center">Enter your new password below.</p>
